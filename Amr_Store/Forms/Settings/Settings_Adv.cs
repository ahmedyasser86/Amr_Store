...
namespace Motaz_Store
{
    public partial class Settings_Adv : Form
    {
        ...
        public bool BarcodeConsecutively = false;
        public Settings_Adv()
        {
            InitializeComponent();
            ...
            BarcodeConsecutively = 
                (Convert.ToInt32(GetDataString("SELECT Value FROM Settings WHERE Setting='BarcodeConsecutively'")) == 1) ?
                true : false;
            ch_BarcodeSync.Checked = BarcodeConsecutively;
        ...
        private void Ch_BarcodeSync_CheckedChanged(object sender, EventArgs e)
        {
            int Value;
            if(ch_BarcodeSync.Checked)
            {
                // BarcodeConsecutively
                Value = 1;
            }
            ...
            {
                // BarcodeNonConsecutively
                Value = 0;
            }

            if (CmdExcuteRows("UPDATE Settings SET Value = '" + Value + "' WHERE Setting = 'BarcodeConsecutively'") == 1)
            {
                msg.ShowError("تم التعديل بنجاح", true);
                BarcodeConsecutively = (Value == 1) ? true : false;
            }
        }
    }
}
