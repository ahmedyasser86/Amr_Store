...
namespace Motaz_Store
{
    public partial class Store_View : Form
    {
        ...
        public Store_View()
        {
            ...
            // Load Data To DGV
            DGVs("SELECT pp.Des الوصف, pp.Art الأرتكل, p.Color اللون, " +
                "SUM((p.Qty - ISNULL(s.QTY, 0) - ISNULL(w.Qty, 0) - ISNULL(o.Qty, 0))) الكمية, " +
                "AVG((pp.Price - pp.Descount)) 'سعر البيع', AVG(pp.F_Price) الجملة, pp.Sup المورد " +
                "FROM Products_Prices pp LEFT JOIN Products p ON p.Art = pp.Art LEFT JOIN Online o " +
                "ON o.P_ID = p.ID LEFT JOIN (SELECT SUM(Qty) Qty, P_ID FROM Sells GROUP BY P_ID) s " +
                "ON s.P_ID = p.ID LEFT JOIN (SELECT SUM(Qty) Qty, P_ID FROM WithDraws GROUP BY P_ID) w " +
                "ON w.P_ID = p.ID WHERE (p.Qty - ISNULL(s.QTY, 0) - ISNULL(w.Qty, 0) - ISNULL(o.Qty, 0)) != 0 " +
                "GROUP BY p.Color, pp.Art, pp.Des, pp.Sup ORDER BY pp.Art", dgv_Sells);

            // Hide F_Price if user not manager
            if(!Session.isManager)
            {
                dgv_Sells.Columns["الجملة"].Visible = false;
            }
            ...
            {
                totalQty += Convert.ToInt32(r.Cells["الكمية"].Value);
            }
            lbl_TotalQty.Text = "إجمالي الكمية: " + totalQty;
            ...
            if (filter)
            {
                ...
                if (ch_Sizes.Checked)
                {
                    ...
                    q = "SELECT p.ID كود, pp.Des الوصف, pp.Art الأرتكل, p.Color اللون, p.Size المقاس, " +
                        "(p.Qty - ISNULL(s.QTY, 0) - ISNULL(w.Qty, 0) - ISNULL(o.Qty, 0)) الكمية, " +
                        "(pp.Price - pp.Descount) 'سعر البيع', (pp.F_Price) الجملة," +
                ...
                else
                {
                    q = "SELECT pp.Des الوصف, pp.Art الأرتكل, p.Color اللون, SUM((p.Qty - ISNULL(s.QTY, 0) - ISNULL(w.Qty, 0) - " +
                        "ISNULL(o.Qty, 0))) الكمية, AVG((pp.Price - pp.Descount)) 'سعر البيع', AVG(pp.F_Price) الجملة, pp.Sup المورد " +
        ...
        private void Btn_Refresh_Click(object sender, EventArgs e)
        {
            ...
            DGVs("SELECT pp.Des الوصف, pp.Art الأرتكل, p.Color اللون, SUM((p.Qty - ISNULL(s.QTY, 0) - ISNULL(w.Qty, 0) - ISNULL(o.Qty, 0)))" +
                " الكمية, AVG((pp.Price - pp.Descount)) 'سعر البيع', AVG(pp.F_Price) الجملة, pp.Sup المورد FROM Products_Prices pp " +
        ...
        {
            if (e.ColumnIndex == 1 && dgv_Sells.Columns.Count == 7)
            {
