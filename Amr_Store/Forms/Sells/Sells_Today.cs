...
namespace Motaz_Store
{
    public partial class Sells_Today : Form
    {
        ...
        private void Btn_Add_Click(object sender, EventArgs e)
        {
            ...
            // Upadate Safe if Ex not from the safe
            if(!ch_FromSafe.Checked)
            {
                try
                {
                    int oldsafe = int.Parse(lbl_Safe.Text);
                    int newsafe = oldsafe + int.Parse(txt_Ex.Text);

                    if (CmdExcuteRows("UPDATE Safe SET Amount=" + newsafe + " WHERE InDay=@InDay"
                        , new string[] { "@InDay", Session.inDay }) != 1)
                        throw new Exception();

                    Session.safe = newsafe;

                    lbl_Safe.Text = newsafe.ToString();

                    ch_FromSafe.Checked = true;
                }
                catch
                {
                    msg.ShowError("فشل تعديل الخزينة");
                    return;
                }
            }
