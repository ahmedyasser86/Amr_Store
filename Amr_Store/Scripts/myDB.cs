...
namespace Motaz_Store
{
    public class myDB
    {
        ...
        {
            if(Forms.settings_Adv.BarcodeConsecutively)
            {
                Task.Run(() =>
                {
            ...
            else
            {
                foreach (Sizes s in sizes)
                {
                    for (int i = 0; i < s.Qty; i++)
                    {
                        ...
                        // Print barcode for this size
                        printBarcodeConsecutively(code, art, color, s.Size, price);
                    }
                }
            }
        }
        ...
        public static void printBarcodeConsecutively(int code, string art, string color, int size, int price)
        {
            Reports.DataSets.BarCode ds = new Reports.DataSets.BarCode();
